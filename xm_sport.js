...
const XM_UP = process.env.XM_UP // 用户名&密码
let username, password

const $ = new Env(name)
if (XM_UP) {
    const up = XM_UP.split("&")
    username = up[0]
    password = up[1]
}
if (!username || !password) {
    console.log('❌ 请先提供小米运动账号，eg. 176xxxx6128&11xxxx')
    process.exit(-1)
}
        ...
        $.post(options, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (data.result === 200) {
                        console.log(`刷步成功 --> ${data.message}`)
                    } else {
                        console.log(`刷步失败 --> ${data.message}`)
                    }
